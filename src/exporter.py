...
def exporter_pour_algorithmes(geojson_rues_path):
    """
    Utility module to transform unified GeoJSON data into an in-memory 
    ...
    for index, row in gdf.iterrows():
        # Handle potential list types in highway column from OSMnx
        ...
        # Build standard arc dictionary
        arc = {
            "id_arc": int(row.get("key", index)),
            "noeud_source": int(row.get("u", -1)),
            "noeud_cible": int(row.get("v", -1)),
            "quartier": str(row.get("quartier", "inconnu")),
            "type_route": type_route,
            "distance_km": float(row.get("length_km", 0.0)),
            "priorites": {
                "securitaire": bool(row.get("is_crit_security", False)),
                "social": bool(row.get("is_crit_social", False)),
                "economique": bool(row.get("is_crit_economique", False))
            },
            "scenarios": {
